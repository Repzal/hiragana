...
let quizQ = null;
let quizChoices = [];
let quizPool = []; // Will be set at start of quiz to selected kana for session
...
function startQuiz() {
  ...
  // quizPool is now a static snapshot of current selected kana for this session
  quizPool = flat.filter(h => selected.includes(h.kana));
  nextQuizQ();
}
function stopQuiz() {
  quizMode = false;
...
function nextQuizQ() {
  ...
  const distractors = shuffle(
    flat.filter(h => h.kana !== answer.kana && quizPool.map(k=>k.kana).indexOf(h.kana) === -1)
  ).slice(0, Math.max(0, 3 - (quizPool.length - 1)));
  // Always have 4 choices: use all (non-answer) quizPool entries as distractors if enough, else fill with chart
  let otherQuizPool = shuffle(quizPool.filter(h => h.kana !== answer.kana)).slice(0, 3);
  while (otherQuizPool.length < 3) {
    // supplement with distractors from outside
    if (distractors.length) otherQuizPool.push(distractors.pop());
    else break;
  }
  const choices = shuffle([answer, ...otherQuizPool]);
  quizQ = answer;
  quizChoices = choices;
